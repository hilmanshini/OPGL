 ...
 * been moved.
 */
public class RangeBar extends View implements ViewTreeObserver.OnGlobalLayoutListener {
...
//            mConnectingLine.draw(canvas, mLeftThumb, mRightThumb);
            mRBackgroundBarLeft.draw(canvas, 0, 0, mLeftThumb.getX(), getHeight());
            mBackgroundBarRight.draw(canvas, mRightThumb.getX(), mRightThumb.getY(), getWidth(), getHeight());
            if (drawTicks) {
//                mBar.drawTicks(canvas);
    ...
    // Private Methods /////////////////////////////////////////////////////////
    BackgroundBar mRBackgroundBarLeft, mBackgroundBarRight;
    ...
    private void rangeBarInit(Context context, AttributeSet attrs) {
        ...
        mBackgroundBarRight = new BackgroundBar();
        mRBackgroundBarLeft = new BackgroundBar();
    ...
    public void onGlobalLayout() {
        ...
        mediaMetadataRetriever.setDataSource(videoPath);
        int viwdth = Integer.parseInt(mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH));
        int vheight = Integer.parseInt(mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT));
        float ratio = (float) getHeight() / (float) vheight;
        int sbmtWidth = (int) (viwdth * ratio);
        long duration = Long.parseLong(mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION));
        setTickStart(0);
        setTickEnd(duration);
        setTickInterval(1);
        int sbmtHeighht = (getHeight());
        int capacity = getWidth() / sbmtWidth;
        int factorDuration = (int) (duration / capacity);
        Log.e("TESSS", "EVERY BITMAP SHOULD HAVE w:" + (viwdth * ratio) + " " + getHeight() + " capacity " + capacity + " perseconds: " + factorDuration + "/" + duration);

        if (mOnMeasureVideoChange != null) {
            mOnMeasureVideoChange.onGetValueMeasue((int) (viwdth * ratio), getHeight(), capacity, factorDuration);
        }
        getViewTreeObserver().removeOnGlobalLayoutListener(this);
    }

    OnMeasureVideoChange mOnMeasureVideoChange;

    public void setOnMeasureVideoChange(OnMeasureVideoChange mOnMeasureVideoChange) {
        this.mOnMeasureVideoChange = mOnMeasureVideoChange;
    }

    public interface OnMeasureVideoChange {
